-- Corrected RLS Policies for extended schema tables
-- This file fixes the column reference issues
...
DROP POLICY IF EXISTS "Users can create applications" ON adoption_applications;
DROP POLICY IF EXISTS "Users can update their pending applications" ON adoption_applications;
...
DROP POLICY IF EXISTS "Users can send messages in their applications" ON user_messages;
DROP POLICY IF EXISTS "Admins can send messages in any application" ON user_messages;
DROP POLICY IF EXISTS "Users can update read status of messages sent to them" ON user_messages;
...
-- Note: admin_stats is a view and inherits security from underlying tables
